...
use webkit2gtk::{
  ...
  TLSErrorsPolicy, URIRequest, URIRequestExt, URISchemeRequest, URISchemeRequestExt,
  URISchemeResponse, URISchemeResponseExt, WebContext, WebContextExt as Webkit2gtkContextExt,
  WebView, WebViewExt, WebsiteDataManagerExt,
};
...
impl WebContextImpl {
  ...
  pub fn create_context(context: WebContext) -> Self {
    ...
    Self::apply_operator_tls_policy(&context);
  ...
  /// Self-hosted BetterDesk panels may use HTTP, self-signed, or incomplete-chain HTTPS.
  fn apply_operator_tls_policy(context: &WebContext) {
    if std::env::var("BETTERDESK_TLS_STRICT").ok().as_deref() == Some("1") {
      return;
    }
