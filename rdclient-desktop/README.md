...
| **Remote → local (drag-out)** | Start dragging files on the remote (**keep the mouse button down**). RdClient immediately shows a local “copy” cursor and downloads the files; when ready a native OLE drag starts (`ReleaseCapture` + click-through + `SHDoDragDrop`) so you can drop on Desktop/Explorer/taskbar. Do not release until the OLE drag cursor appears. Plain **Copy → Paste** still works without dragging out |
