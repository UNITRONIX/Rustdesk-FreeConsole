...
### Fixed
- **RdClient — File transfer button in fullscreen:** toolbar notch stayed reachable in viewer-shell fullscreen, and the File transfer modal mounts inside `#rd-viewer-shell` (Fullscreen API hides `body` children). Clicking the folder control no longer silently no-ops when the modal/session is unavailable (toast). Ships via panel update.
