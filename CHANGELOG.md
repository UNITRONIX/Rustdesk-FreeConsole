...
- **RdClient desktop — Settings / Connect white freeze (Windows):** secondary windows no longer create a second WebView2 environment against the shared user-data folder (mismatched `CoreWebView2EnvironmentOptions` left a blank window and hung the UI). Settings and session windows now reuse the main window’s environment on the UI thread; browser args also keep wry’s default disable-features list. Rebuild `rdclient-desktop` (`npm run dev` / `npm run build`).
