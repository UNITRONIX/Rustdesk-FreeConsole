...
### Fixed
- **Stock RustDesk — address book card shows ID instead of panel name:** peer cards use `alias` as the bold title (else formatted ID) and `username@hostname` (+ `note`) on the secondary line. BetterDesk now maps panel **Display name** (else **Note**) into RustDesk `alias`, hides the computer hostname on that secondary line when a panel label is set, and clears a duplicate `note` so labels like “Training” are not shown twice. Stock RustDesk cannot put the ID on the secondary line. Ships via panel update (console + Go rebuild/restart); re-login or refresh address book in RustDesk. Set Devices → Display name for the title you want.
