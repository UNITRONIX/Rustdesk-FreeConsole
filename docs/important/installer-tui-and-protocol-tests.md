...
- Cert choice: 1=keep existing (auto-repairs LE symlinks), 2=self-signed (RSA4096+SAN), 3=Let's Encrypt (certbot standalone + **copy** to `$RUSTDESK_PATH/ssl/` + deploy renew hook), 4=custom paths (validates X.509). Calls `apply_console_protocol_mode` + `run_protocol_tests` after restart. `do_configure_ssl` also calls `run_protocol_tests`.
