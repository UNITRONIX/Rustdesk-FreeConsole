...
- **Windows NSSM stop → deploy → start:** Before replacing `betterdesk-server.exe`, the panel stops `BetterDeskServer`, deploys, then starts. Do not use bare `nssm restart` as the primary path. `SERVICE_PAUSED` (restart throttle) is cleared via `nssm continue`. Verify `SERVICE_RUNNING` after start.
