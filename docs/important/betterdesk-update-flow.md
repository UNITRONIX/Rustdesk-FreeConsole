...
- **Windows agent-source sync:** Stage under `<consoleRoot>/agent-source/` (not hardcoded `/opt/...` → `C:\opt\...`). Writes clear read-only/EPERM on overwrite. Orphan trees under `C:\opt\BetterDeskConsole\agent-source` from older builds can be deleted manually.
