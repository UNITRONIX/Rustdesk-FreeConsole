...
- **Windows path root (#272):** `resolveProjectRoot()` must never resolve to a drive root (`C:\`). Default layout `C:\BetterDeskConsole` + `C:\BetterDesk` writes Scripts & Docker files under the console directory. `ensureParentDirForFile()` skips `mkdir` on filesystem roots (Node throws `EPERM` on `mkdir('C:\\')`). NSSM OpenService Access Denied when controlling `BetterDeskServer`: probe `nssm status` and/or Go `/api/health`; if the service is already running, treat as success and recommend a panel refresh — do not show a failed install. Manual `betterdesk.ps1` restart only if health is down.
- **Windows agent-source sync:** Stage under `<consoleRoot>/agent-source/`. Never probe Linux `/opt/...` on Windows (orphan `C:\opt\BetterDeskConsole\agent-source` with `build.sh` used to win and keep hitting EPERM). Stale `AGENT_SOURCE_DIR` under `C:\opt\...` is ignored. Writes clear read-only/EPERM on overwrite. Orphan `C:\opt\BetterDeskConsole` trees can be deleted manually.
- **Windows NSSM stop → deploy → start:** Before replacing `betterdesk-server.exe`, the panel stops `BetterDeskServer`, deploys, then starts. Do not use bare `nssm restart` as the primary path. `SERVICE_PAUSED` (restart throttle) is cleared via `nssm continue`. If `nssm stop` leaves `SERVICE_RUNNING`, escalate with `sc stop` and `taskkill` on the service PID / Application exe; **do not** deploy the new binary over a live process — fail that step critically and tell the operator to stop/kill then re-Apply. Verify `SERVICE_RUNNING` after start.
